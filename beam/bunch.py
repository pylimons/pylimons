...
class Bunch():
    ...
    def propagate_twiss_parameters(self, twiss, mat):
        ...
        new_twiss[1] = beta0 * mat[_x, _x] ** 2 + alpha0 * (-2 * mat[_x, _x] * mat[_x, _xp]) + gamma0 * mat[
            _x, _xp] ** 2
        new_twiss[0] = beta0 * (-mat[_x, _x] * mat[_xp, _x]) + alpha0 * (
                    mat[_x, _x] * mat[_xp, _xp] + mat[_x, _xp] * mat[_xp, _x]) + gamma0 * (
                                   -mat[_x, _xp] * mat[_xp, _xp])
    ...
    def generate_transverse_matched_beam_distribution(self, seed_no=None):
        ...
        mean = np.zeros((4,),
                        'd')  # [np.sqrt(cov_mat[_x,_x]), np.sqrt(cov_mat[_xp,_xp]), np.sqrt(cov_mat[_y,_y]), np.sqrt(cov_mat[_yp,_yp])]
